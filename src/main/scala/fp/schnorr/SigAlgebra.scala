...
import tsec.signature.jca.{JCASigAlgebra, SigCertificate, SigPrivateKey, SigPublicKey}
...
abstract class JCASigInterpreter[F[_], A](algorithm: String)(implicit M: Sync[F])
  ...
  def genSignatureInstance: F[SchnorrSig] = M.delay(Signature.getInstance(algorithm))
